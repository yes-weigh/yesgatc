...
describe('roleCanOpenCertificates', () => {
  it('denies every portal role', () => {
    for (const role of ROLES) {
      assert.equal(roleCanOpenCertificates(role), false);
    }
...
describe('navSpecsForRole Certificates', () => {
  it('hides Certificates nav for every role', () => {
    for (const role of ROLES) {
      assert.equal(labels(role, true).includes('Certificates'), false);
    }
  });

  it('does not expose certificates paths', () => {
    for (const role of ROLES) {
      const paths = navSpecsForRole(role, { hasCreatedVerifiers: true }).map(item => item.path);
      assert.equal(paths.some(path => path.includes('/certificates')), false);
    }
  });
...
describe('navSpecsForRole Vr Allotted', () => {
  ...
  it('places Vr Allotted as a top-level item after Verification when RC created ≥1 verifier', () => {
    ...
    assert.equal(specs[verificationIdx + 1]?.path, '/rc/vr-allotted');
    assert.equal(specs[verificationIdx + 1]?.label, 'Vr Allotted');
    assert.equal(specs[verificationIdx + 2]?.path, '/rc/customers');
  ...
  it('keeps Verification then Vr Allotted as siblings', () => {
    const specs = navSpecsForRole('rc_admin', { hasCreatedVerifiers: true });
    const verificationIdx = specs.findIndex(item => item.path === '/rc/verification');
    assert.equal(specs[verificationIdx + 1]?.label, 'Vr Allotted');
    assert.equal(specs[verificationIdx + 2]?.label, 'Customers');
  });
});
