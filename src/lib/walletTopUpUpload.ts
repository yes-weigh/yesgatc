...
export async function uploadWalletTopUpScreenshot(
  topUpId: string,
  file: File,
  ...
  if (!topUpId.trim()) throw new Error('Top-up id is required to upload screenshot.');

  await ensureUploadAuth();

  const ext = file.name.includes('.') ? file.name.slice(file.name.lastIndexOf('.')) : '';
  const path = `walletTopUps/${topUpId}/screenshot/${Date.now()}${ext}`;
