...
export function validateRcPdfSignerSignFile(file: File): string | null {
  if (!['image/png', 'image/jpeg'].includes(file.type)) {
    return 'Signature must be a JPG or PNG image.';
  }
  if (file.size > MAX_RC_IMAGE_BYTES) {
    return 'Signature file must be 15 MB or smaller.';
  }
  return null;
}

export async function uploadRcPdfSignerSign(
  rcUid: string,
  file: File,
  onProgress?: (percent: number) => void,
): Promise<ProductFileMeta> {
  const validation = validateRcPdfSignerSignFile(file);
  if (validation) throw new Error(validation);
  if (!rcUid.trim()) throw new Error('Save the regional center first to upload the signature.');

  await ensureUploadAuth();

  const ext = file.name.includes('.') ? file.name.slice(file.name.lastIndexOf('.')) : '.png';
  const path = `users/${rcUid}/pdf-signer/${Date.now()}${ext}`;
  const storageRef = ref(storage, path);
  const task = uploadBytesResumable(storageRef, file, { contentType: file.type });
