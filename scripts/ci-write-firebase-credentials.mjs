...
const rawJson = (
  process.env.FIREBASE_SERVICE_ACCOUNT_YESGATC?.trim() ||
...
if (!rawJson && !rawB64 && !firebaseToken) {
  ...
  console.error('::error::  FIREBASE_SERVICE_ACCOUNT_YESGATC — service account JSON (existing hosting secret)');
  console.error('::error::  or FIREBASE_SERVICE_ACCOUNT — service account JSON');
...
if (rawJson || rawB64) {
  ...
  try {
    ...
    console.error('::error::Grant this account Firebase Admin + Cloud Scheduler Admin on yesgatc.');
    process.exit(1);
  }
