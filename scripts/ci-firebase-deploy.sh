...
# --force on functions: delete orphans removed from source (non-interactive).
FORCE_FLAG=()
case "$TARGETS" in
...
while [ "$attempt" -le "$MAX_ATTEMPTS" ]; do
  ...
  if [ "$attempt" -eq "$MAX_ATTEMPTS" ]; then
    ...
    echo "::error::If the log shows \"Premature close\" on oauth2/token, pin Node to 22.23.1+ (nodejs/node#63989)."
    echo "::error::If the log shows HTTP 503 from firebaserules.googleapis.com, re-run the workflow."
    echo "::error::If the log shows HTTP 409 on firebaserules releases, another deploy raced — re-run once."
    exit 1
  fi
